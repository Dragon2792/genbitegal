...
export async function addArtikel(formData: FormData) {
  try {
    ...
    const slug = judul?.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '') || 'no-slug';

    await prisma.tbl_tulisan.create({
      data: {
        tulisan_judul: judul || 'Tanpa Judul',
        tulisan_isi: isi || '',
        tulisan_author: author || 'Admin',
    ...
    console.error("ADD ARTIKEL ERROR:", err);
...
export async function editArtikel(id: number, formData: FormData) {
  try {
    ...
    console.error("EDIT ARTIKEL ERROR:", err);
    return { success: false, error: err.message || String(err) };
  }
